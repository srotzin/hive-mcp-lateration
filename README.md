...
| `verify_receipt` | Verify offline (always free) — re-derives the root from the asserted event incl. `avoided_cost` | `event`, `claims_root` |
| `settle_scouts` | Compute the cross-tenant scout settlement (spread + scout share) | `settlement_notional_usd` |
| `get_pubkey` | Return the ML-DSA-65 (NIST FIPS 204) public key + issuer metadata | — |
